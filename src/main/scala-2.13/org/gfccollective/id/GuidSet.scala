...
case class GuidSet[A] private (highBits: Array[Long], lowBits: Array[Long]) extends Set[Guid[A]] {
  ...
  override def incl(elem: Guid[A]): Set[Guid[A]] = sys.error("Too inefficient to support")

  override def excl(elem: Guid[A]): Set[Guid[A]] = sys.error("Too inefficient to support")
